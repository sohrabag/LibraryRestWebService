...
public class LoanController {
	...
	public List<Loan> getLoansByMemberId()
	{
		return lms.findAll();
	}
/**
 * Query method returns list of Loan object to query all the loans with
 * method names that corresponds to a sql query such as below.
 * @param bookId
 * @return
 */
	@GetMapping("/loans//books/{bookId}/all")
	public List<Loan> getLoansByBookId(@PathVariable int bookId)
	{
		return lms.getLoansByBookId(bookId);
	}

	@GetMapping("/member/loans/{memberId}/all")
	public List<Loan> getAllByMemberId(@PathVariable int memberId)
	{
		return lms.getAllByMemberId1(memberId);
	}
	...
	public Loan updateLoan(@PathVariable int memberId, @PathVariable int loanId, @Valid @RequestBody Loan loanRequest)
	{
		...
		return lms.update( loanId, loanRequest);
	}
